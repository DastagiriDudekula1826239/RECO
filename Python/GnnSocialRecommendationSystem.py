...
class GnnSocialRecommendationSystem(nn.Module):
    ...
    def forward(self, nodes_u, nodes_v):
        ...
        u_latent = self.user_user_updater(nodes_u)
        ...
        predScores = torch.cat((u_latent_factor_repr, item_latent_repr), 1)
...
def main():
    ...
    # data loading
    user_to_items_history, user_ratings_to_items_history, item_by_users_history, item_ratings_by_users_history, train_u, train_v, train_r, test_u, test_v, test_r, user_to_users_social_adjacency, ratings_list = pickle.load(open(properties.getProperty('SOCIAL_RECO_DATASET'), 'rb'))

    # Validation dataset hardcoded by taking 10% from train dataset
    validation_u = train_u[12891:len(train_u)]
    validation_v = train_v[12891:len(train_v)]
    validation_r = train_r[12891:len(train_r)]

    train_Dataset = torch.utils.data.TensorDataset(torch.LongTensor(train_u), torch.LongTensor(train_v), torch.FloatTensor(train_r))
    validation_Dataset = torch.utils.data.TensorDataset(torch.LongTensor(validation_u), torch.LongTensor(validation_v), torch.FloatTensor(validation_r))
    test_Dataset = torch.utils.data.TensorDataset(torch.LongTensor(test_u), torch.LongTensor(test_v), torch.FloatTensor(test_r))
    ...
    """
    user_item_aggregator = UserItemAggregator(user_embeddings, item__embeddings, rating__embeddings, embeddig_dim, cuda=device, user_latent_flag=True)
    user_item_updater = UserItemUpdater(user_to_items_history, user_ratings_to_items_history, user_item_aggregator, user_embeddings, embeddig_dim)

    user_user_aggregator = UserUserAggregator(lambda nodes: user_item_updater(nodes).t(), user_embeddings, embeddig_dim, cuda=device)
    user_user_updater = UserUserUpdater(lambda nodes: user_item_updater(nodes).t(), user_to_users_social_adjacency, user_user_aggregator, embeddig_dim, userFeatsUVFlag, cuda=device)

    """
    to read the item latent for user-item network
    """
    item_aggregator = UserItemAggregator(user_embeddings, item__embeddings, rating__embeddings, embeddig_dim, cuda=device, user_latent_flag=False)
    item_updater = UserItemUpdater(item_by_users_history, item_ratings_by_users_history, item_aggregator, item__embeddings, embeddig_dim)

    """
    base model
    """
    reco = GnnSocialRecommendationSystem(user_item_updater, user_user_updater, item_updater).to(device)
